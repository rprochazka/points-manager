...
export class PointsService {
  ...
  addPoint(pointRecord: IPointRecord): Observable<void> {
    ...
    return this.httpClient
      .post<void>(this.getBaseUlr(), pointRecord)
      .pipe(catchError(this.handleError));
  }

  editPoint(pointRecord: IPointRecord): Observable<void> {
    pointRecord.lastModifiedDate = new Date().toJSON();
    return this.httpClient
      .put<void>(`${this.getBaseUlr()}/${pointRecord.pointId}`, pointRecord)
      .pipe(catchError(this.handleError));
  }

  getPoint(pointId: string): Observable<IPointRecord> {
    return this.httpClient
      .get<IPointRecord>(`${this.getBaseUlr()}/${pointId}`)
      .pipe(catchError(this.handleError));
  }
  ...
  private handleSearching(pointsFilter: PointsFilter): string {
    if (
      pointsFilter.queryOptions &&
      pointsFilter.queryOptions.searchExpression
    ) {
  ...
  private handlePaging(pointsFilter: PointsFilter): string {
    ...
    if (
      pointsFilter.queryOptions &&
      pointsFilter.queryOptions.take &&
      pointsFilter.queryOptions.take > 0
    ) {
      urlParams.push(`_limit=${pointsFilter.queryOptions.take}`);
    }
    if (
      pointsFilter.queryOptions &&
      pointsFilter.queryOptions.skip &&
      pointsFilter.queryOptions.take > 0
    ) {
